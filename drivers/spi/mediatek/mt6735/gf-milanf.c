...
static gf_dev_t gf;
/* add by goodix 20160602 for nav start*/
...
/* add by goodix 20160602 for nav end*/
...
static struct pinctrl_state *pins_poweron_ldo0, *pins_poweroff_ldo0, *pins_poweron_ldo1, *pins_poweroff_ldo1;
...
struct gf_configs fdt_down_cfg[]=
{
	...
	{GF_FDT_DELTA,0x157F},//goodix change 20160815
...
struct gf_configs fdt_up_cfg[]=
{
	...
	{GF_MODE_CTRL2,0x0014}, 
	{GF_PIXEL_CTRL6,0x0100}, 
	{GF_FDT_DELTA,0x137F}, //goodix change 20160815
	...
	if(onoff) {
...
#endif
	             pinctrl_select_state(pinctrl1, pins_poweron_ldo1);
		...
		if (1 == gf_dev->poweron){
			...
			//regulator_disable(reg);
	                pinctrl_select_state(pinctrl1, pins_poweroff_ldo1);
			gf_dev->poweron = 0;
			msleep(5);
		}
	}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_CMD:
		...
		gf_debug(DEFAULT_DEBUG,"%s GF_IOC_CMD command is %x \n",__func__,command);
		mutex_lock(&gf_dev->buf_lock);
		gf_spi_send_cmd(gf_dev,&command,1);
		mutex_unlock(&gf_dev->buf_lock);
		mdelay(2); //goodix change 20160815
	    break;
	case GF_IOC_CONFIG:
		retval = __get_user(config_type, (u32 __user*)arg);
		gf_debug(DEFAULT_DEBUG, "%s GF_IOC_CONFIG config_type is %d ",__func__,config_type);		
	...
	case GF_IOC_SENDKEY:
		...
		for(i = 0; i< ARRAY_SIZE(key_map); i++) {
/*add by chenhui for fingerprinter key, begin*/
		gf_debug(DEFAULT_DEBUG, "%s key %d, value %d",__func__, gf_key.key, gf_key.value);
			
            if(gf_key.key == KEY_UP || gf_key.key == KEY_DOWN 
				||gf_key.key == KEY_LEFT ||gf_key.key == KEY_RIGHT/*key_map[i].val == gf_key.key*/){

               if(gf_key.key == KEY_UP)
                {
			if(gf_key.value == 1)
			{
			input_report_key(gf_dev->input, KEY_FINGER_UP, 1);
			input_sync(gf_dev->input);
			} else {
			input_report_key(gf_dev->input, KEY_FINGER_UP, 0);
                input_sync(gf_dev->input);
                }
                }
		  else if(gf_key.key == KEY_DOWN)
                {
			if(gf_key.value == 1)
			{
			input_report_key(gf_dev->input, KEY_FINGER_DOWN, 1);
			input_sync(gf_dev->input);
			} else {
			input_report_key(gf_dev->input, KEY_FINGER_DOWN, 0);
			input_sync(gf_dev->input);					
                }	
                }	
		  else if(gf_key.key == KEY_LEFT)
                {
			if(gf_key.value == 1)
			{
			input_report_key(gf_dev->input, KEY_FINGER_LEFT, 1);
			input_sync(gf_dev->input);
			} else {
			input_report_key(gf_dev->input, KEY_FINGER_LEFT, 0);
			input_sync(gf_dev->input);					
                }	
                }	
		  else if(gf_key.key == KEY_RIGHT)
                {
			if(gf_key.value == 1)
			{
			input_report_key(gf_dev->input, KEY_FINGER_RIGHT, 1);
			input_sync(gf_dev->input);
			} else {
			input_report_key(gf_dev->input, KEY_FINGER_RIGHT, 0);
			input_sync(gf_dev->input);					
                }	
                }	
/*add by chenhui for fingerprinter key, end*/		  
...
static int gf_open(struct inode *inode, struct file *filp)
{
    gf_dev_t *gf_dev;
    int status = -ENXIO;
    int cnt = 0; //goodix change 20160815
    unsigned short reg =0; //goodix change 20160815
    ...
    if(status == 0){
		...
		if(status == 0) {
			...
			#if 1 //goodix change 20160815 start
			/* 1. disable irq */
			//disable_irq(fingerprint_irq);
			...
			while(cnt < 10) {
			   ...
			   //gf_debug(DEFAULT_DEBUG,"%s IRQ status : %x \n",__func__,reg);	
			...
			#endif //goodix change 20160815 end

			/* 4. open device node and open irq*/
		    nonseekable_open(inode, filp);
		    gf_debug(DEFAULT_DEBUG, "Succeed to open device. irq = %d", fingerprint_irq);
		    //mt_eint_unmask(gf_dev->spi->irq);
		    enable_irq(fingerprint_irq);	
		}
	} else {
...
static int gf_release(struct inode *inode, struct file *filp)
{
    ...
    gf_dev->users --;
    if(!gf_dev->users) {
		gf_debug(DEFAULT_DEBUG, "disable_irq. irq = %d", fingerprint_irq);
		//mt_eint_mask(gf_dev->spi->irq);
		disable_irq(fingerprint_irq);
    }
    mutex_unlock(&device_list_lock);
...
//void fingerprint_gpio_as_int(int pin)
//{
//	//mutex_lock(&fingerprint_set_gpio_mutex);
//	//fingerprint_DEBUG("[fingerprint]fingerprint_gpio_as_int\n");
//	if (pin == 1)
//		pinctrl_select_state(pinctrl1, eint_as_int);
//	//mutex_unlock(&fingerprint_set_gpio_mutex);
//}
//
//void fingerprint_gpio_output(int pin, int level)
//{
//	//mutex_lock(&fingerprint_set_gpio_mutex);
//	//fingerprint_DEBUG("[fingerprint]fingerprint_gpio_output pin = %d, level = %d\n", pin, level);
//	if (pin == 1) {
//		if (level)
//			pinctrl_select_state(pinctrl1, eint_pulldisable);
//		else
...
int fingerprint_get_gpio_info(struct platform_device *pdev)
{
	...
	pins_poweron_ldo0 = pinctrl_lookup_state(pinctrl1, "fp_state_ic_poweron_ldo0");
	if (IS_ERR(pins_poweron_ldo0)) {
		ret = PTR_ERR(pins_poweron_ldo0);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_ic_poweron_ldo0!\n");
		return ret;
	}	
	pins_poweroff_ldo0 = pinctrl_lookup_state(pinctrl1, "fp_state_ic_poweroff_ldo0");
	if (IS_ERR(pins_poweroff_ldo0)) {
		ret = PTR_ERR(pins_poweroff_ldo0);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_ic_poweroff_ldo0!\n");
		return ret;
	}	
	pins_poweron_ldo1 = pinctrl_lookup_state(pinctrl1, "fp_state_ic_poweron_ldo1");
	if (IS_ERR(pins_poweron_ldo1)) {
		ret = PTR_ERR(pins_poweron_ldo1);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_ic_poweron_ldo1!\n");
		return ret;
	}	
	pins_poweroff_ldo1 = pinctrl_lookup_state(pinctrl1, "fp_state_ic_poweroff_ldo1");
	if (IS_ERR(pins_poweroff_ldo1)) {
		ret = PTR_ERR(pins_poweroff_ldo1);
		dev_err(&pdev->dev, "fwq Cannot find nasri_fingerprint pinctrl state_ic_poweroff_ldo1!\n");
		return ret;
	}		
		

	spi_state = pinctrl_lookup_state(pinctrl1, "spi1_cs_set");
	pinctrl_select_state(pinctrl1, spi_state);
	spi_state = pinctrl_lookup_state(pinctrl1, "spi1_clk_set");
	pinctrl_select_state(pinctrl1, spi_state);
	ps_miso_spi = pinctrl_lookup_state(pinctrl1, "spi1_miso_set");
	pinctrl_select_state(pinctrl1, ps_miso_spi);
	spi_state = pinctrl_lookup_state(pinctrl1, "spi1_mosi_set");
	pinctrl_select_state(pinctrl1, spi_state);
	ps_miso_pullup = pinctrl_lookup_state(pinctrl1, "spi1_miso_pullup");
...
static int  gf_probe(struct spi_device *spi)
{
	...
	pinctrl_select_state(pinctrl1, pins_poweron_ldo0);
	//printk("nasri...%s %d\n",__func__,__LINE__);
	...
	/*SPI parameters.*/	
	gf_spi_setup(gf_dev, 6*1000*1000);//1*1000*1000
	gf_spi_set_mode(gf_dev->spi, SPEED_6MHZ, 0);//SPEED_1MHZ
	//printk("nasri...%s %d\n",__func__,__LINE__);
	...
	if (node) {
		...
		printk("irq number %d \n", fingerprint_irq);			
		ret = request_irq(fingerprint_irq, (irq_handler_t) fingerprint_eint_interrupt_handler, IRQF_TRIGGER_RISING,
					"FINGERPRINT-eint", NULL);
	}
