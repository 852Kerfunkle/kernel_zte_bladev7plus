...
BATTERY_VOLTAGE_ENUM g_cv_voltage = BATTERY_VOLT_04_400000_V;//BATTERY_VOLT_04_340000_V;
#else
...
static void set_jeita_charging_current(void)
{
	...
	if(BMT_status.charger_type == STANDARD_CHARGER){
		...
		printk("[BATTERY] JEITA set charging current : %d, input current : %d, battery status : %d\r\n",
			g_temp_CC_value,g_temp_input_CC_value,g_temp_status);
	}
#else
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
...
#endif
	battery_log(BAT_LOG_CRTI,
			    "[BATTERY] [mt_battery_charging_algorithm] bat_charging_state = 0x%x\n",BMT_status.bat_charging_state);
