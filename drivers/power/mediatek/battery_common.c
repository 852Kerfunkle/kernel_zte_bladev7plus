...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if (chr_wake_up_bat == KAL_FALSE) {
			...
			else if ((BMT_status.UI_SOC - BMT_status.SOC) >= 5){
				BMT_status.UI_SOC--;
				timer_counter = 0;
			}
#ifdef FG_BAT_INT
...
static void battery_update(struct battery_data *bat_data)
{
	...
	mdelay(100);
	if(battery_meter_get_charger_voltage() < 4250)
	{
...
void mt_battery_GetBatteryData(void)
{
	...
	if(BMT_status.temperature > 50 && BMT_status.temperature <= 65)
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	else if ((BMT_status.temperature < TEMP_NEG_10_THRESHOLD) && BMT_status.charger_exist) {
...
void BAT_thread(void)
{
	...
	if(BMT_status.charger_type == NONSTANDARD_CHARGER){
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &BMT_status.charger_type);
		printk("BMT_status.charger_type = %d\n",BMT_status.charger_type);
		if ((BMT_status.charger_type == STANDARD_HOST)
			|| (BMT_status.charger_type == CHARGING_HOST)) {
			mt_usb_connect();
		}
