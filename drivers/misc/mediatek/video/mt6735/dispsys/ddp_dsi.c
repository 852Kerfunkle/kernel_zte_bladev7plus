...
//add by yujianhua for esd check
int esd_check_init = 0x0;
//add by yujianhua for esd check end
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
	struct DSI_RX_DATA_REG read_data0;
//add by yujianhua for esd check start
DSI_T0_INS t1;
struct DSI_RX_DATA_REG read_data1;
struct DSI_RX_DATA_REG read_data2;
struct DSI_RX_DATA_REG read_data3;
unsigned int  data_arr[4] = {0,0,0,0};
static unsigned int  data_paras[2] = {0x0f,0xf0};//only for hx8394f
int j=0;
...
//add by yujianhua for esd check end
			...
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0],
				      AS_UINT32(&t1));//add by yujianhua for esd check 0x00013700
...
*/
	if(hSlot[0])//(hSlot)
	{
	// read from slot cmdqBackupReadSlot(hSlot, i, ((uint32_t *) &read_data0));
	...
	DISPCHECK("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,
		dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1],dsi_params->lcm_esd_check_table[i].para_list[2]);
	packet_type = read_data0.byte0;
		if(packet_type == 0x1A || packet_type == 0x1C)
		{
			recv_data_cnt = read_data0.byte1 + read_data0.byte2 * 16;
			...
			if(ret == 1)
			{
				DISPCHECK("DSI read long packet data error i=%d \n",i);
				esd_check_init = 0x1;
				break;
			 }
		}
		else
		{
			...
			{
				DISPCHECK("DSI read short packet data error i=%d \n",i);
				ret = 1; // esd fail
				esd_check_init = 0x1;
				break;
			}
